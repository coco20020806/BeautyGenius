...
import { sample1CollectedDetail, sample1CoverImage } from '../fixtures/collected/sample1Bundle';
import { getPartPresetTutorial, partCovers } from '../fixtures/parts/partsBundle';
import type {
  AdjustmentRequest,
...
const tutorial: IllustratedTutorial = {
  ...
  steps: [
    ...
    { id: 'eyes', order: 4, title: '眼影打底', part: 'eyes', product: '裸粉眼影', color: '#c98586', instruction: '从睫毛根部向上晕染，范围不超过眼窝。', expertTip: '眼尾颜色略深即可。', videoSlice: '02:07–04:18' },
...
const assets: CuratedAsset[] = [
  ...
  { id: 'contour-soft', title: '柔和轮廓修容', category: 'part', part: 'contour', source: '知识库部位素材', style: '自然', occasion: '日常', difficulty: '新手', color: '#b18b7b', practiced: true, coverImage: partCovers.contour, tutorialId: 'preset-contour-soft', placements: ['library', 'mix'] },
  { id: 'lips-rose', title: '低饱和玫瑰唇', category: 'part', part: 'lips', source: '知识库部位素材', style: '清透', occasion: '通勤', difficulty: '新手', color: '#a94f5b', practiced: false, coverImage: partCovers.lips, tutorialId: 'preset-lips-rose', placements: ['library', 'mix'] },
...
class LocalLearningService implements LearningService {
  ...
  async getTutorial(tutorialId?: string) {
    ...
    const partPreset = getPartPresetTutorial(requestedId);
    if (partPreset) return rememberTutorial(partPreset);
